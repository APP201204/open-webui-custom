...
from open_webui.utils.docx_translation import get_user_language_from_metadata, translate_docx
...
async def process_tool_result(
    ...
    # Translation: Translate .docx files generated by execute_python_code
    if tool_function_name == 'execute_python_code' and tool_result_files:
        user_language = get_user_language_from_metadata(metadata)
        if user_language:
            from open_webui.env import ENABLE_TRANSLATION

            if ENABLE_TRANSLATION:
                for file_info in tool_result_files:
                    file_url = file_info.get('url', '')
                    # Ensure file_url is a string
                    if isinstance(file_url, bytes):
                        file_url = file_url.decode('utf-8')
                    if file_url and file_url.endswith('.docx'):
                        try:
                            log.info(f'Detected .docx file from execute_python_code, translating to {user_language}')

                            # Check if this is a local file URL
                            if file_url.startswith('/api/v1/files/data/'):
                                ...
                                if file and file.path:
                                    ...
                                    if file_path.exists():
                                        ...
                                        translated_path = await translate_docx(str(file_path), user_language)

                                        # Create a new file entry for the translated version
                                        # For now, we'll just log the translated path
                                        # In a full implementation, you would:
                                        # 1. Upload the translated file to storage
                                        # 2. Create a new file record
                                        # 3. Update file_info['url'] to point to the new file
                                        log.info(f'Translated .docx file saved to: {translated_path}')
                                        # TODO: Upload translated file and update file_info['url']
                        except Exception as e:
...
async def background_tasks_handler(ctx):
    ...
    if message and 'model' in message:
        if tasks and messages:
            ...
            if not chat_id.startswith('local:') and not chat_id.startswith('channel:'):  # Only update titles and tags for non-temp chats
...
async def non_streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter:
        try:
            ...
            if 'selected_model_id' in response_data and not chat_id.startswith('channel:'):
                await Chats.upsert_message_to_chat_by_id_and_message_id(
                    metadata['chat_id'],
                    metadata['message_id'],
                    {
                            ...
                            if data:
                                ...
                                # Check for Responses API events (type field starts with "response.")
                                    ...
                                    if value:
                                        ...
                                        if ENABLE_REALTIME_CHAT_SAVE and not chat_id.startswith('channel:'):
                                            # Save message in the database
                                            await Chats.upsert_message_to_chat_by_id_and_message_id(
                                                metadata['chat_id'],
                                                metadata['message_id'],
